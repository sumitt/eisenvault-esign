...
<#macro invitationModeratedTaskDetail task>
   ...
   <#assign theUser = people.getPerson(task.properties["imwf:inviteeUserName"])>
   {
	 ...
	 <#elseif theUser.assocs["cm:esign"]??>
	    "esignRef": "${theUser.assocs["cm:esign"][0].nodeRef?string}",
         </#if>
...
<#macro invitationNominatedTaskDetail task>
   {
	 ...
	 <#elseif theInviter.assocs["cm:esign"]??>
	    "esignRef": "${theInviter.assocs["cm:esign"][0].nodeRef?string}",
         </#if>
