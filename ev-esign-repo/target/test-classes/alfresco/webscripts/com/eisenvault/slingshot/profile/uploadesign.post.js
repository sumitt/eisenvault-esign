...
function main()
{
   try
   {
      ...
      // ensure cm:person has 'cm:preferencesESign' aspect applied - as we want to add the esign as
      // the child node of the 'cm:preferenceESignImage' association
      if (!user.hasAspect("cm:preferencesESign"))
      {
         user.addAspect("cm:preferencesESign");
      }
      ...
      var image = user.createNode(filename, "cm:content", "cm:preferenceESignImage");
      ...
      // wire up 'cm:esign' target association - backward compatible with JSF web-client esign
      assocs = user.associations["cm:esign"];
      if (assocs != null && assocs.length == 1)
      {
         user.removeAssociation(assocs[0], "cm:esign");
      }
      user.createAssociation(image, "cm:esign");
