...
(function()
{
   ...
   YAHOO.extend(Alfresco.UserProfile, Alfresco.component.Base,
   {
       ...
       * @param p_obj {object} Object passed back from addListener method
       */
      onSignUpload: function UP_onSignUpload(e, p_obj)
      {
         if (this.fileUpload === null)
         {
            this.fileUpload = Alfresco.getFileUploadInstance();
         ...
         var uploadConfig =
         {
            flashUploadURL: "slingshot/profile/uploadesign",
            htmlUploadURL: "slingshot/profile/uploadesign.html",
            username: this.options.userId,
            mode: this.fileUpload.MODE_SINGLE_UPLOAD,
            onFileUploadComplete:
            {
               fn: this.onSignUploadComplete,
               scope: this
            }
         };
         this.fileUpload.show(uploadConfig);
       ...
       * File Upload complete event handler
       *
       * @method onSignUploadComplete
       * @param complete {object} Object literal containing details of successful and failed uploads
       */
      onSignUploadComplete: function UP_onSignUploadComplete(complete)
      {
         var success = complete.successful.length;
         if (success > 0)
         {
            ...
            {
               photos[i].src = Alfresco.constants.PROXY_URI + "api/node/" + noderef.replace("://", "/") +
                               "/content/thumbnails/esign?c=force";
            }
            
            // call to update the user object - photo changes take effect immediately!
            var json = {};
            json[this.id + "-photoref1"] = noderef;
            var config =
            {
               method: "POST",
               url: Dom.get(this.id + "-form").attributes.action.nodeValue,
